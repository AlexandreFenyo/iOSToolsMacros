...
// if the Resilient property in Info.plist equals false, an error message is printed and saved, and a fatalError() let the app exit (in case a debugger is attached, a "detach" command in the CLI must be entered to create the crash log on the device). If it is set to true, an error message is printed and saved but the code flow continues to try not to crash.
@freestanding(expression)
